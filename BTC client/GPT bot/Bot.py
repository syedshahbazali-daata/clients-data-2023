...
def pandas_txt(row_text):
    ...
    state = row_text[1]['STATE']
    try:
        state = state_abbreviations[str(state).upper()]
    except:
        state = state



    pandas_record = f"{row_text[1]['NAME']};{row_text[1]['ADDRESS']};{row_text[1]['CITY']};{state};{row_text[1]['ZIPCODE'][:5]}"

    pandas_record = pandas_record.replace('; N/A', '').replace("N/A", '')
    add_row_into_txt('Results/Pandas.txt', pandas_record)


def krishna_txt(row_text):
    """
    Fname Lname
    Address
    City
    ...
    # Krishna.txt (format) Fname Lname; Address; City; State; Zip
    krishna_record = f"{row_text[1]['NAME']}\n{row_text[1]['ADDRESS']}\n{row_text[1]['CITY']}\n{row_text[1]['STATE']}\n{row_text[1]['ZIPCODE'][:5]}\n===================="
    krishna_record = krishna_record.replace('; N/A', '').replace("N/A", '')
    add_row_into_txt('Results/Krishna.txt', krishna_record)
...
def capblack_txt(row_text):
    ...
    capblack_record = f"{capblack_first_name} {capblack_last_name};{row_text[1]['ADDRESS']};{row_text[1]['CITY']};{state};{row_text[1]['ZIPCODE'][:5]}"
