...
def yelp_reviews_scraper(yelp_page_url):
    if "http" not in yelp_page_url:
        return None
    res = requests.get(yelp_page_url)
    data = str(res.text)

    try:
        yelp_id = String(xpath='//meta[@name="yelp-biz-id"]', attr='content').parse_html(data)
        yelp_id = yelp_id[0]
    except:
        return None
    ...
    business_name = ""
    while True:
        reviews_url = f"https://www.yelp.com/biz/{yelp_id}/review_feed?rl=en&q=&sort_by=date_desc&start={start_from}"
        ...
        if len(reviews_data) == 0:
            add_multiple_rows(dataframe, "Sheet1")
            return dataframe
        elif business_name == "":
            business_name = reviews_data[0]["business"]["name"]

        for review in reviews_data:
            index += 1
            review_text = review["comment"]["text"]
            review_date = review["localizedDate"]
            user_name = review["user"]["markupDisplayName"]
            print(review_date)

            try:
                review_date = str(review_date).replace("/", "-")
            except:
                pass

            row = [business_name, "YELP.COM", review_text, user_name, review_date]

            dataframe.append(row)
            print(f"Review {index} scraped successfully ---> YELP")
...
def trip_advisor_reviews_scraper(trip_adv_url):
    ...
    while True:
        ...
        if len(reviews_data) == 0:
            ...
            business_name = data[1]["data"]["locations"][0]["name"]

        for review in reviews_data:
            index += 1
            review_text = review["text"]
            user_name = review["userProfile"]["username"]
            review_date = review["createdDate"]  # 2023-05-19 to mm-dd-yyyy
            try:
                review_date = datetime.strptime(review_date, "%Y-%m-%d").strftime("%m-%d-%Y")
            except:
                pass

            row = [business_name, "TRIPADVISOR.com", review_text, user_name, review_date]
            dataframe.append(row)
            print(row)
...
def booking_review_scraper(main_url):
    ...
    page_name_value = main_url.split("/hotel/us/")[1].split(".html")[0]

    offset = 0
    dataframe = []
    index = 0
    business_name = page_name_value.replace("-", " ").title()
    while True:
        ...
        for single_review in range(total_reviews_container):
            ...
            try:
                # 14 december 2022 to mm-dd-yyyy
                review_date = datetime.strptime(review_date, "%d %B %Y").strftime("%m-%d-%Y")
            except:
                pass

            row = [business_name, "BOOKING.COM", review_text, user_name, review_date]
            index += 1
...
def expedia_review_scraper(main_url):
    ...
    business_name = main_url.lower().split("hotels-")[1].split(".h")[0].replace("-", " ").title()
    while True:
        print(offset, "OFFSET")
        ...
        for single_review in reviews_data:
            ...
            review_date = single_review['submissionTime']['longDateFormat']  # Jun 23, 2023 to mm/dd/yyyy
            try:
                review_date = datetime.strptime(review_date, '%b %d, %Y').strftime('%m/%d/%Y')
            except:
                pass
            user_name = single_review['reviewAuthorAttribution']['text']
            row = [business_name, "EXPEDIA.COM", review_text, user_name, review_date]
...
# yelp_pages = urls_data['yelp']
# for url in yelp_pages:
#     print(f"Scraping {url} ==> Yelp")
#     yelp_reviews_scraper(url)
#
#
# expedia_pages = urls_data['expedia']
# for url in expedia_pages:
#     print(f"Scraping {url} ==> Expedia")
#     expedia_review_scraper(url)
#
# trip_advisor_pages = urls_data['trip advisor']
# for url in trip_advisor_pages:
#     print(f"Scraping {url} ==> Trip Advisor")
#     trip_advisor_reviews_scraper(url)

booking_pages = urls_data['booking.com']
for url in booking_pages:
    print(f"Scraping {url} ==> Booking.com")
    booking_review_scraper(url)
