...
for category in categories:
    ...
    while True:
        url = "https://es.pandora.net/on/demandware.store/Sites-PND-ES-Site/es_ES/Search-UpdateGrid"

        querystring = {"cgid": f"{category}", "start": f"{start}", "sz": "54",
                       "selectedUrl": "https://es.pandora.net/on/demandware.store/Sites-PND-ES-Site/es_ES/Search-UpdateGrid?cgid=charms&start=52&sz=54"}
        ...
        headers = {
            ...
            "referer": "https://es.pandora.net/es/charms-y-pulseras/charms/",
        ...
        urls = String(xpath='//a[@class="link suggestionhit"]', attr='href').parse_html(
            page_source)
        urls = ["https://es.pandora.net" + url for url in urls]
        if len(urls) == 0:
            break
        else:
            print(len(urls))
        ...
        for url in urls:
            record = [category, url]
            if record not in already_scraped:
                with open("urls_data.csv", "a", encoding="utf-8", newline="") as f:
                    csv.writer(f).writerow(record)
                already_scraped.append(record)
                print(record)
