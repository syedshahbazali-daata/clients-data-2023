...
def set_date(year: int, month_no: int, day_no: int):
    specific_clicker('(//input[@class="vdatetime-input form-control g-input"])[1]')

    # Select Year
    specific_clicker('//div[@class="vdatetime-popup__year"]')
    year_new = str(year)[-2:]
    specific_clicker(f'//*[text()="20{year_new} " or text()="{year}"]')
...
def set_time(hour: int, minute: int, am_pm: str):
    ...
    if len(str(minute)) == 1:
        minute = f'0{minute}'
    specific_clicker('(//input[@class="vdatetime-input form-control g-input"])[2]')
...
def post_image_with_schedule(schedule_date: str, schedule_time: str, media: list, set_post_price: int = 0,
    ...
    print([schedule_date, schedule_time, media, set_post_price, set_text, no_of_preview_images])
    ...
    set_time(post_time[0], post_time[1], "AM")
...
def bot_start_only_fans():
    with open('schedules.json') as f:
        config = json.load(f)['schedule_data']
        data = [schedule for schedule in config if schedule['platform'] == 'onlyfans']
...
def bot_start_Twitter():
    ...
    for index, single_post in enumerate(data):
        ...
        # SET POST TIME: 07:07 convert to 7:07 AM or 7:07 PM according to the time
        time_post = data[index]['post_time'].split(':')


        # --> check date and time is more than current date and time
        if int(year_post) < int(datetime.now().year) and int(month_post) < int(datetime.now().month) and int(day_post) < int(datetime.now().day):
            print("Date is less than current date")
            continue
        elif int(year_post) == int(datetime.now().year) and int(month_post) < int(datetime.now().month) and int(day_post) < int(datetime.now().day):
            # check time
            if int(time_post[0]) < int(datetime.now().hour) and int(time_post[1]) < int(datetime.now().minute):
                print("Time is less than current time")
                continue
        ...
        while True:
            try:
                ...
                driver.find_element(By.XPATH, f'//*[@id="SELECTOR_2"]//*[text()="{day_post}" or text()="{int(day_post)}"]').click()
                break
            except Exception as e:
                print(e)
                time.sleep(1)
        ...
        hour_post = int(time_post.split(':')[0])
        minute_post = str(time_post.split(':')[1].split(' ')[0])
        am_pm_post = time_post.split(':')[1].split(' ')[1]

        # SET POST HOUR
        while True:
            try:
                driver.find_element(By.XPATH, f'//*[@id="SELECTOR_4"]//*[text()="0{hour_post}" or text()="{int(hour_post)}"]').click()
                break
            except Exception as e:
                print(e)
                time.sleep(1)
        ...
        while True:
            try:
                driver.find_element(By.XPATH, f'//*[@id="SELECTOR_5"]//*[text()="0{minute_post}" or text()="{int(minute_post)}"]').click()
                break
            except Exception as e:
                print(e)
                time.sleep(1)
        ...
        for i in range(4):
            try:
                driver.find_element(By.XPATH, f'//*[@id="SELECTOR_6"]//*[text()="{am_pm_post}"]').click()
                break
            except Exception as e:
                print(e)
                time.sleep(1)
