...
def instagram_uploader(text, file):
    ...
    time.sleep(2)
    for x in range(5):
        specific_clicker2("//button[text()='OK']")
...
# add the options to improve performance
...
while True:
    for single_row in df.iterrows():
        ...
        my_date = str(upload_datetime.split(" ")[0])
        my_time = str(upload_datetime.split(" ")[1][:5])

        # get the current time
        current_date = str(datetime.now()).split(" ")[0]
        current_time = str(datetime.now()).split(" ")[1][:5]

        already_done = get_file_data("Already done")
        # if the current time is greater than the upload time, then upload the video
        print(current_time, my_time, current_date, my_date, platform)
        time.sleep(1)
        if current_time == my_time and path not in already_done and current_date == my_date:
            if platform == 'FACEBOOK':
                print('Uploading to facebook')

                facebook_upload_post(caption, path)
            if platform == 'TIKTOK':
                print('Uploading to tiktok')
                tiktok_upload_post(caption, path)

            if platform == 'INSTAGRAM':
                print('Uploading to instagram')
                instagram_uploader(caption, path)
