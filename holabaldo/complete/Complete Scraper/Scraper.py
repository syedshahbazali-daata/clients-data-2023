...
def insert_record(record:tuple):
...
# count total rows: cursor.execute("SELECT COUNT(*) FROM listings")
# Database is Connected Successfully
print("Database is connected successfully")
#
# wrong_urls = get_file_data("wrong records.txt")
# for url in wrong_urls:
#     print(url)
#     sql = f"DELETE FROM listings WHERE url = '{url}'"
#     cursor.execute(sql)
#     connection.commit()
...
for row in records:
    ...
    url_x = row[0]
    title_x = row[1]
    if len(str(title_x).strip()) < 5:
        indexes.append(row[0])
        # save in Already_scraper.txt
        with open("wrong records.txt", 'a', encoding='utf-8') as file:
            file.write(url_x + '\n')
...
# delete a record from the table URL_extra_data and listings where url https://www.easybroker.com/agent/mls_properties/10-000-m2-cerca-a-aeropuerto-de-queretaro


# sql = "DELETE FROM listings WHERE url = 'https://www.easybroker.com/agent/mls_properties/10-000-m2-cerca-a-aeropuerto-de-queretaro'"
# cursor.execute(sql)
# connection.commit()
# print(cursor.rowcount, "record(s) deleted")
...
for url in list_of_urls:
    ...
    title = get_text_pass('//div[@class="property-title"]')
    ...
    record = (url, title, price, description, features, address, zip_code, neighborhood, images)
    print(record)
    try:
