...
def make_dir(name: str):
    BASE_DIR = os.getcwd()
    path = os.path.join(BASE_DIR, "WhiteHorse", name)
...
driver.get('https://www.whitehorse.vic.gov.au/planning-building/advertised-documents-and-registers')

# Pre-work
while True:
    try:
        advertisement_url = driver.find_element(By.XPATH,
                                                "//*[text()='application documents and plans are available to be viewed online']").get_attribute(
            'href')
...
time.sleep(5)
specific_clicker('//input[@aria-label="Planning Register - at Advertising"]')
...
# Main work
applications_urls_list = []
while True:
    applications_urls = driver.find_elements(By.XPATH, '//table[@id="gridResults"]//td/div/a')

    if len(applications_urls) > 0:
        applications_urls = [url.get_attribute("href") for url in applications_urls]
        applications_urls_list.extend(applications_urls)

    else:
        time.sleep(2)

    try:
        driver.find_element(By.XPATH, '//input[contains(@title, "Next")]')
        specific_clicker('//input[contains(@title, "Next")]')
    except:
        break

print(f'Found {len(applications_urls_list)} applications in total')

for application_url in applications_urls:
    df = pd.read_csv('../ScraperLogs.csv')
    already_done = df[df['Scraper'] == 'WhiteHorse']['Tracking ID'].tolist()
    ...
    try:
        applications_address = driver.find_element(By.XPATH, "//*[text()='Site Address']/following-sibling::*").text
    except:
    ...
    # //td//a[@title]
    pdfs = driver.find_elements(By.XPATH, "//td//a[text()='Download...']")
    pdfs = [pdf.get_attribute("href") for pdf in pdfs]
    print(f"Found {len(pdfs)} PDF's")

    for download_url in pdfs:
        make_dir(applications_number)
        download_file_path = os.path.join("WhiteHorse", applications_number, download_url.split('=')[-1])
        download_file_path = os.getcwd() + "\\" + download_file_path + ".pdf"
        download_file(download_url, download_file_path)

        s3_path = f"WhiteHorse/{applications_number}/{download_url.split('=')[-1]}.pdf"
    ...
    record = (max_id, applications_number, applications_address, applications_description, "", "",
              "WhiteHorse", "", "", "", "")
    add_row_to_table("ScrapersData", record)
    print(f"Added to database: {applications_number}")
    ...
    with open('../ScraperLogs.csv', 'a', newline='', encoding='utf-8') as f:
        ...
        csv_writer.writerow([application_url, "WhiteHorse", current_date, current_time])

driver.quit()
clear_download_folder("WhiteHorse")
